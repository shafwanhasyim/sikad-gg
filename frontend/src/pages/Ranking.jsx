    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="p-4 bg-blue-50 border-b border-blue-100">
          <h2 className="text-lg font-semibold text-blue-800">Academic Performance Ranking</h2>
          <p className="text-sm text-blue-600">Based on overall grade point average across all courses</p>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {ranking.map((student, index) => (
                  <tr key={student.mahasiswa._id || index} className={index < 3 ? "bg-yellow-50" : ""}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{student.mahasiswa.npm}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{student.mahasiswa.name}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{student.mahasiswa.jurusan}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 inline-flex text-sm leading-5 font-semibold rounded-full 
                        ${student.ip >= 3.5 ? "bg-green-100 text-green-800" : 
                        student.ip >= 3.0 ? "bg-blue-100 text-blue-800" :
                        student.ip >= 2.5 ? "bg-yellow-100 text-yellow-800" :
                        "bg-red-100 text-red-800"}`}>
                        {student.ip.toFixed(2)}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <Link to={`/mahasiswa/${student.mahasiswa._id}`} className="text-blue-600 hover:text-blue-900">
                        View Details
                      </Link>
                    </td>
                  </tr>
