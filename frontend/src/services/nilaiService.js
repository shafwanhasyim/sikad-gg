...
const nilaiService = {
  ...
  getNilaiByMahasiswa: async (mahasiswaId) => {
    try {
      ...
      const response = await api.get(`/nilai/getNilaiByMahasiswa/${mahasiswaId}`);
      return Array.isArray(response.data) ? response.data : [];
    } catch (error) {
      console.error(`Failed to get nilai for mahasiswa with ID ${mahasiswaId}:`, error.message);
      return [];
    }
  },
  ...
  getNilaiByMataKuliah: async (mataKuliahId) => {
    try {
      ...
      const response = await api.get(`/nilai/getNilaiByMataKuliah/${mataKuliahId}`);
      return Array.isArray(response.data) ? response.data : [];
    } catch (error) {
      console.error(`Failed to get nilai for mata kuliah with ID ${mataKuliahId}:`, error.message);
      return [];
    }
  },
    // Get IP semester for a mahasiswa
  getIpSemester: async (mahasiswaId) => {
    try {
      if (!mahasiswaId) throw new Error("Mahasiswa ID is required");
      const response = await api.get(`/nilai/ipSemester/${mahasiswaId}`);
      return response.data && typeof response.data === 'object' && 'ips' in response.data 
        ? response.data.ips 
        : null;
    } catch (error) {
      console.error(`Failed to get IP semester for mahasiswa with ID ${mahasiswaId}:`, error.message);
      return null;
    }
  },
  ...
  getDistribusi: async (mataKuliahId) => {
    try {
      ...
      return response.data && response.data.distribusi ? response.data.distribusi : {};
    } catch (error) {
      console.error(`Failed to get distribution for mata kuliah with ID ${mataKuliahId}:`, error.message);
      return {};
    }
  }
};
